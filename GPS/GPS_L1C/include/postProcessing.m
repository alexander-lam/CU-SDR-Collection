...
if ((settings.skipAcquisition == 0) || ~exist('acqResults', 'var'))
    ...
    codeLen = max(42,2*settings.acqNonCohTime);
...
%--- Display local time of first position solution in UTC -----------------
if ~isempty(navResults)
    for i = 1:length(eph)
        if eph(i).health == 0
            weekNumber = eph(i).weekNumber;
            break;
        end
    end
    disp(['UTC Time of First Position Solution: ' char(gpst2utcg(weekNumber, navResults.localTime(1)), 'dd-MMM-yyyy HH:mm:ss.SSSSSSSSS')]);
    
    %--- Display actual sampling frequency
    disp(['Actual Sampling Frequency: ' ...
        num2str((navResults.currMeasSample(end)-navResults.currMeasSample(1))/(navResults.localTime(end)-navResults.localTime(1))) ...
        'Hz']);
end
